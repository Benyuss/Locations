...
public class GeneralController {
	...
	public String customInput(@ModelAttribute("custom") Location customInput, ModelMap model) {
		...
		String geohash = locCalc.generateGeohash(customInput.getLatitude(), customInput.getLongitude(),
				customInput.getRadius());

		if (repository.findOne(geohash) == null) {
			repository.save(new LocationDB(customInput.getLatitude(), customInput.getLongitude(),
					customInput.getRadius(), geohash));
		} else {
	...
	public String selectrecord(Model model) {
		...
		List<LocationDB> locations = new ArrayList<LocationDB>();
		for (LocationDB temploc : repository.findAll()) {
			locations.add(temploc);
		}
		model.addAttribute("queryValue", locations);
	...
	public String parseHash(@ModelAttribute("geohash") String geohash, ModelMap model) {
		...
		ArrayList<LocationDB> locations = new ArrayList<LocationDB>(); // all
																		// except
																		// that
		for (LocationDB temploc : repository.findAll()) { // fill that list with
															// that data but
															// exclude the
															// chosen one.
		...
		ArrayList<PairedData> data = locCalc.calculate(locations, loc); // for
																		// html
																		// (geohash.jsp)
																		// parse
	...
	@PostMapping(value = "/geohash", params = "index") // back to index with
														// keeping DB alive.
	public ModelAndView geohash2index() {
		return new ModelAndView("redirect:index");
	}

	@PostMapping(value = "/geohash", params = "reset") // reset database and
														// back to index.
